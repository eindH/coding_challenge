...
client = pymongo.MongoClient("mongodb+srv://admin:adminPassword@cluster0.ldmkj.mongodb.net/propertyManager?retryWrites=true&w=majority")
db = client.propertyManager
userCol = db.Users
propertyCol = db.Properties
app = FastAPI()

class User(BaseModel):  #DEFINES A USER MODEL
    first_name: Optional[str] = None
    last_name: Optional[str] = None

class Property(BaseModel):  #DEFINES A PROPERTY MODEL
...
@app.post('/reg-user')
async def regUser(user: User):    #REGISTER USER BY ENTERING FIRST AND LAST NAME
    new_user = userCol.insert_one({'first_name' : user.first_name, 'last_name': user.last_name})
    return {"message": user.first_name + ' ' + user.last_name + ' ADDED WITH ID# ' + str(new_user.inserted_id)}

@app.get('/list-all-users')
async def listAllUsers(): #GET CURSOR FOR COLLECTION AND LOOP THRU
    cursor = userCol.find({})
    user_dict = {}
    for doc in cursor:
        user_dict[str(doc['_id'])] = {'first_name': doc['first_name'], 'last_name': doc['last_name']}
    return user_dict

@app.get('/user-by-id/{user_id}')
async def userById(user_id):  
    user_details = userCol.find_one({"_id": ObjectId(str(user_id))})
    return {'first_name' : user_details['first_name'], 'last_name': user_details['last_name']}

@app.post('/create-property')
async def createProperty(propertyDoc: Property):
    new_property = propertyCol.insert_one({'address1': propertyDoc.address1,'address2': propertyDoc.address2,'city': propertyDoc.city,'postcode': propertyDoc.postcode, 'value': propertyDoc.value,'owner': propertyDoc.owner})
    return {"message": "PROPERTY ADDED WITH ID#" + str(new_property.inserted_id)}
...
async def findUserProperties(user_id):
    ...
    property_dict = {}
    for doc in cursor:
        property_dict[str(doc['_id'])] = {'address1': doc['address1'],'address2': doc['address2'],'city': doc['city'],'postcode': doc['postcode'], 'value': doc['value'],'owner': doc['owner']}
...
async def updateUser(user_id, updated_details: User):
    ...
    userCol.find_one_and_update({"_id": ObjectId(str(user_id))}, {"$set": { 'first_name': update_first_name, 'last_name': update_last_name}})
...
async def updateProperty(property_id, updated_details: Property):
    ...
    propertyCol.find_one_and_update({"_id": ObjectId(str(property_id))}, {"$set": { 'address1': update_address1, 'address2': update_address2, 'city': update_city, 'postcode': update_postcode, 'value': update_value, 'owner': property_doc['owner']}})
...
async def updatePropertyOwner(property_id, updated_details: Property):
    ...
    propertyCol.find_one_and_update({"_id": ObjectId(str(property_id))}, {"$set": {'owner': update_owner}})
